...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val occurences = w.toLowerCase groupBy (x => x)
    val charOccurenceCount = occurences map (x => (x._1, x._2.length))
    charOccurenceCount.toList.sorted
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val difference = x.foldLeft(Map[Char, Int]())((acc, pair) => {
      acc.updated(pair._1, pair._2 - y.toMap.withDefaultValue(0)(pair._1))
    })
    difference.filter(l => l._2 > 0).toList.sortBy(pair => pair._1)
  }
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] = occurrences match { 
      ...
      case _ => {
        for {
          ...
          r <- anagrams(subtract(occurrences, c))
        } yield w :: r
      }
    }
